...
- **An integration suite that boots a real Laravel application.**
  ...
  container binding and facade, the `certigniter::` view namespace, and the
  facade root `QrCodeRenderer` needs, which no suite reached before.
...
- **CI** (`.github/workflows/ci.yml`): both suites across Laravel 12 and 13
  ...
  `composer audit`. Support for a framework version is now something the
  repository asserts on every push and weekly, rather than something
  somebody once checked by hand.
...
- **Error messages now say what to do.** Every rejection in `IgniterPackage`
  ...
  repacked archive, a file from a newer Design Studio, a full disk and a
  missing `ext-zip` each say what happened and what fixes it.
